...
function showUpdateGetGoodsBox(Applies, method) {
	if(Applies==null){
		Applies=document.getElementById("uid").value;
	}else {
		var datetime=GMTToStr(Applies.gdate)
		var appliesGname=Applies.gname;
	}
	// 检查当前页面是否弹层，如果有则删除
...
/*	overtimeHtml += '<div class="form-group">';
	...
	overtimeHtml += '<div class="col-sm-10">';
	if(appliesGname!=null){
		overtimeHtml += '<input type="text" class="form-control" id="goods" value="'+appliesGname+'">';
	}else {
	...
	if (method =='add') {
		overtimeHtml += '<input type="button" class="btn btn-primary btn" value="添加" onclick="addgoods('+Applies+')"/>';
	} else{
		overtimeHtml += '<input type="button" class="btn btn-primary btn" value="修改" onclick="updgoods('+Applies.gid+')"/>';
	}
...
//修改
function updgoods(gid) {
	var res =  validate();
	if (!res) return; 
	gname=$("#goods").val()
	datetime=$("#atime").val()
			$.ajax({
			url:"/apply/updateGoods",
			type:"POST",
			data:{
			"gid":gid,
			"gname":gname,
			...
			success:function(data){
				if(data.indexOf("成功")!=-1){
					alert("修改成功")
					window.location.href = "/apply/officeApply/null/1";	
				}else {
					alert("修改失败，请稍后重试")
				}
				}
...
function selectName() {
	...
	if(realName == ""){
	 ...
	 window.location.href = "/apply/officeApply/" + realName+"/"+1;
		}	