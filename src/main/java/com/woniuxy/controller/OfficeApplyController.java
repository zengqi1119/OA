...
public class OfficeApplyController {
	...
	public String officeSubscribe(@PathVariable("placeholder") String uname, Model model, HttpServletRequest request,
			Buygoods buygoods, @PathVariable("currentPage") Integer currentPage,HttpSession session) {
		int currentpage = currentPage;
		int count = 8;
		Integer rid = (Integer) session.getAttribute("rid");
		List<Integer> uid=new ArrayList<Integer>();
		PageBean<Getgoods> officeApplies=null;
		if(rid==4) {
			...
			if (uname != null && !(uname.equals("null"))) { // 前端传了个null字符串
				uid = officeApplyService.selectUidByUname(uname);
				if (uid.size() == 0) {
					uid.add(0); // 0代表没有这个人
				}
			}
		}
		officeApplies = officeApplyService.selectAllMessage(currentpage, count, uid);
		// 翻页url
